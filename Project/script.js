...
window.addEventListener("DOMContentLoaded", function () {
  ...
  // IMPORTANT: match the id of the camera entity in HTML.
  camera = document.querySelector("#head");

  // Safety check.
  if (!camera) {
    console.error("Camera entity #head not found!");
    return;
  }
...
function updateAmmoDisplay() {
  ...
  if (vr) {
    vr.setAttribute(
      "text",
      "value: Ammo: " + ammo_count + "; color: red; align: center; width: 2"
    );
  }
}
...
function loop() {
  ...
  for (let a = 0; a < ammo_boxes.length; a++) {
    ...
    let dist = Math.sqrt(
      Math.pow(playerPos.x - ammoPos.x, 2) +
      Math.pow(playerPos.y - ammoPos.y, 2) +
      Math.pow(playerPos.z - ammoPos.z, 2)
    );
    if (dist < 2) {
  ...
  for (let b = 0; b < bullets.length; b++) {
    ...
    if (
      Math.abs(bullet.obj.object3D.position.x) > 150 ||
